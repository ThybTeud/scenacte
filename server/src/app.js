    ...
    cors({
        origin: (origin, callback) => {
            ...
            if (isAllowed) {
                ...
                console.warn(
                    `[CORS] 💡 Origines autorisées: ${allowedOrigins
                        .filter((o) => o)
                        .join(", ")}`
                );
                console.warn(
                    `[CORS] 💡 Configurez CLIENT_URL=${origin} sur Render si c'est votre frontend`
                );
...
app.use(
    express.urlencoded({
        extended: true,
        limit: `${config.limits.maxContentSizeMB}mb`,
    })
);
...
function registerErrorHandlers() {
    ...
    const shouldServeFrontend =
        ["production", "staging"].includes(config.server.env) &&
    ...
    } else if (["production", "staging"].includes(config.server.env)) {
        console.log("[STATIC] Frontend serving disabled (backend-only mode)");
        console.log(
            "[STATIC] Set SERVE_FRONTEND=true to enable static file serving"
        );
    }
